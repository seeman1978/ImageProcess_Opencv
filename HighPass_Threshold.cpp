...
/// usage: prog {<algorith> <image_name> }
...
int main(int argc, char** argv)
{
    ...
    const string lowPassName = "High Pass Filtered and Threshold (grayscale)"; // window name
    ...
    if(( argc == 3 && (!(img = imread( argv[2], IMREAD_COLOR)).empty()))||
       ( argc == 3 && (cap.open(argv[2]) == true )) ||
       ( argc < 3 && (cap.open(CAMERA_INDEX) == true)))
    {
        ...
        while (keepProcessing) {
            ...
            std::string strAlgorithm = argv[1];
            if (strAlgorithm.compare("butterworth") == 0)
            {
                create_butterworth_highpass_filter(filter, radius, order);
            }
            else if(strAlgorithm.compare("gaussian") == 0)
            {
                create_gaussian_highpass_filter(filter, radius);
            }
            else
            {
                std::cerr << "ERROR: cannot get algorithm name" << std::endl;
                exit(0);
            }
            /// apply filter
            shiftDFT(complexImg);
