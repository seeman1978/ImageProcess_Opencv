//
// Created by zhaoyue on 2019/11/15.
//
...
int main(int argc, char** argv)
{
       ...
       ( argc != 2 && (cap.open(CAMERA_INDEX) == true)))
    {
        ...
        namedWindow("After Histogram Equalization", 0);
        ...
        while (keepProcessing) {
            ...
            Mat imgHigh_Frequency_Emphasis;
            filter.convertTo(imgHigh_Frequency_Emphasis, -1, k2, k1);        //公式：k1+k2*H(u, v)

            /// apply filter
            shiftDFT(complexImg);
            mulSpectrums(complexImg, imgHigh_Frequency_Emphasis, complexImg, 0);
            shiftDFT(complexImg);
            ...
            Mat dest;
            imgOutput.convertTo(dest, CV_8UC1, 256, 0);     //imgOutput归一化【0，1】，需要乘以256，然后在转换到8bit。equalizeHist需要bit mat作为输入
            ...
            imshow("After Histogram Equalization", dest);
