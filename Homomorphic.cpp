//
// Created by zhaoyue on 2019/11/19.
//
...
/// create a 2-channel gaussian low-pass filter with radius D
/// (assumes pre-aollocated size of dft_Filter specifies dimensions)
void create_gaussian_highpass_filter_2(Mat& dft_Filter, double L, double H, double C, int D)
{
    Mat tmp = Mat(dft_Filter.rows, dft_Filter.cols, CV_32F);
    Point centre = Point(dft_Filter.rows/2, dft_Filter.cols/2);
    double radius_pow2;
    ...
    for (int i = 0; i < dft_Filter.rows; ++i) {
        for (int j = 0; j < dft_Filter.cols; ++j) {
            radius_pow2 = (double)(pow((double)(i - centre.x), 2.0) + pow((double)(j - centre.y), 2.0));     //D(u, v)的平方
            tmp.at<float>(i, j) = (H-L)*(1 - (double)exp((double)(-C*radius_pow2/pow((double)D, 2.0))))+L;
        }
    }
    Mat toMerge [] = {tmp, tmp};
    merge(toMerge, 2, dft_Filter);
